...
namespace JGMSoftware
{
    class WebContentHelper
    {
        public static string HtmlHeader(double viewportWidth, double height) //adapt parametres
        {
            ...
            head.Append(string.Format("h2{{font-size: 28px; font-weight:lighter;}} " +
            ...
            "width: 100%;" +
            "z-index: 0;}}" +
            "article{{column-fill: auto;column-gap: 90px;column-width: 690px; column-height:100%; height:"));
            head.Append(height);
            head.Append(string.Format("px}}" +
            "img,p.object,iframe {{ max-width:690px; height:auto }}"));
